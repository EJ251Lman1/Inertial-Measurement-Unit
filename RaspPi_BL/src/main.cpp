...
// Upload data to ThingSpeak (7 fields)
void sendDataToThingSpeak(double temp, double humidity, double bpm, double spo2,
    ...
    std::string apiKey = "EJ9ZFBN17ZA7HBOS"; // Your API key
    std::string url = "http://api.thingspeak.com/update?api_key=" + apiKey +
                      "&field1=" + std::to_string(temp) +
                      "&field2=" + std::to_string(humidity) +
                      "&field3=" + std::to_string(bpm) +
                      "&field4=" + std::to_string(spo2) +
                      "&field5=" + std::to_string(ax) +
                      "&field6=" + std::to_string(ay) +
                      "&field7=" + std::to_string(az);
...
int main() {
    ...
    while (true) {
        ...
        if (rx_length > 0) {
            ...
            double temp, humidity, bpm, spo2, ax, ay, az;
            if (sscanf((const char*)rx_buffer, "%lf %lf %lf %lf %lf %lf %lf",
                       &temp, &humidity, &bpm, &spo2, &ax, &ay, &az) == 7) {

                auto now = std::chrono::steady_clock::now();
                if (std::chrono::duration_cast<std::chrono::seconds>(now - lastUpdate).count() >= 15) {
                    sendDataToThingSpeak(temp, humidity, bpm, spo2, ax, ay, az);
                    lastUpdate = now;
                }
            }
        }
